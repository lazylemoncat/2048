...
class env(gym.Env):
    ...
    def move(self, action):
        if action == "w":
            self.matrix, done, score = Move.up(self.matrix)
        elif action == "s":
            self.matrix, done, score = Move.down(self.matrix)
        elif action == "a":
            self.matrix, done, score = Move.left(self.matrix)
        elif action == "d":
            self.matrix, done, score = Move.right(self.matrix)
        if self.gameState.game_state(self.matrix) != 'not over':
            self.__done = True
        if done and not self.__done:
            self.matrix = self.add_two(self.matrix)
        return score
    ...
    def getReward(self, old_state, new_state, score):
        reward = 0
        if self.are_arrays_equal(old_state, new_state):
            return -1  # 轻微惩罚无效移动
        reward += score  # 奖励得分
        old_max_value = max(old_state)
        new_max_value = max(new_state)
        if new_max_value > old_max_value:
            reward += (new_max_value - old_max_value) * 10  # 奖励合并产生更高瓦片
        if new_max_value == 11:  # 2048 = 2^11
            reward += 1000  # 奖励合成2048
        return reward
    ...
    def step(self, action):
        ...
        self.state = self.flatten_matrix()
        reward = self.getReward(old_state, self.state, score)
        if self.__done:
            reward -= 500  # 惩罚游戏结束
        info = {"score": score}
        if self.window is not None:
